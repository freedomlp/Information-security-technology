...
}; //S-盒是一类选择函数，用于二进制6-4转换。Feistel轮函数使用8个S-盒S1, …, S8。
...
void bin_to_dec(int block[][64], int a[], int block_num)//2进制转换10进制
{
    ...
    for(int i = 0; i < block_num; ++i)
    {
        ...
        {
            a[count] = block[i][j] * 128 + block[i][j + 1] * 64 + block[i][j + 2] * 32 + block[i][j + 3] * 16
                       + block[i][j + 4] * 8 + block[i][j + 5] * 4 + block[i][j + 6] * 2 + block[i][j + 7] * 1;
            ++count;
         }
    }
}
...
char *read()//读入一行字符串
{
    int str_len = 10; //假设输入字符串长度不超过10
    char *input = (char *)malloc(sizeof(char) * str_len);
    char c;
    int count = 0;
    while ((c = getchar()) != '\n')
    {
        ...
        if (count > str_len)
        {
            str_len = str_len * 2; //每次不够用了就两倍的扩展内存空间
            input = (char *)realloc(input, sizeof(char) * str_len);
        }
        input[count - 1] = c;
    }
    input[count] = '\0';
...
int main()
{
    ...
    char mode;
    char *input, *key_16;
    printf("choose your mode, encryption/E, decryption/D: ");
    scanf("%c", &mode);
    getchar();
    printf("input your plaintext/ciphertext: ");
    input = read();
    printf("input your key: ");
    key_16 = read();    
    hex_to_bin(key_16, key);//key作为16进制读入后先转换为二进制
    get_sub_key(key, sub_key);
    if(mode == 'E')
    {
        block_num = (strlen(input) / 8) + 1;
        char ciphertext[block_num * 16 + 1];
        int output[block_num][64];
        encryption_or_decryption(input, output, sub_key, block_num, mode);
        bin_to_hex(output, ciphertext, block_num);
        printf("%s", ciphertext);
    }
    else
    {
        ...
        int output[block_num][64], reverse_sub_key[16][48];
        for(int i = 0; i < 16; ++i)
        {
            for(int j = 0; j < 48; ++j)
                reverse_sub_key[i][j] = sub_key[15 - i][j];
        }
        encryption_or_decryption(input, output, reverse_sub_key, block_num, mode);
        bin_to_dec(output, plaintext, block_num);
        int fill = output[block_num- 1][60] * 8 + output[block_num - 1][61] * 4 + output[block_num - 1][62] * 2 + output[block_num - 1][63];
